...
void DFS(int* candidates,
    ...
    if (remain->remainTarget == 0) {
        ...
        for (; remain->now != 0; remain->now--) {
            ...
            DFS(candidates, remain, list, returnSize, returnColumnSizes,
                *(candidates + remain->now - 1));
            if (sizeof(list) != 0) {
                    ...
                    *returnSize++;
                    int** nptr =
                        realloc(returnColumnSizes,
                                sizeof(returnColumnSizes) + sizeof(int**));
                    if (!nptr)
                        printf("realloc error\n");
                    returnColumnSizes = nptr;
                    *(*returnColumnSizes +
                      sizeof(*returnColumnSizes) / sizeof(int*) - 1) =
